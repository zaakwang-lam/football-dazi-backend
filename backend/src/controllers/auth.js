...
async function adminLogin(req, res) {
  ...
  res.json(success({
    accessToken: generateAccessToken(payload), refreshToken: generateRefreshToken(payload),
    admin: { id: admin.id, username: admin.username, role: admin.role, realName: admin.realName, courtId: admin.courtId }
  }));
}
...
async function userLogin(req, res) {
  ...
  res.json(success({
    ...
    user: {
      ...
      role: user.role, roles: user.roles || [], courtId: user.courtId,
      registered: Array.isArray(user.roles) ? user.roles.length > 0 : false
    }
  }));
}
...
async function registerRole(req, res) {
  ...
  if (role === 'court') {
    ...
    if (!surfaceTypes.length) surfaceTypes.push('人工草地');
    ...
    try {
      court = await Court.create({
        name: courtInfo.name, ownerId: userId, type: courtInfo.type, address: courtInfo.address, district: courtInfo.district,
        longitude: courtInfo.longitude ? Number(courtInfo.longitude) : null, latitude: courtInfo.latitude ? Number(courtInfo.latitude) : null,
        ...
        surfaceType: surfaceTypes[0], surfaceTypes, openHours, description: courtInfo.description || '', status: 2
      });
    } catch (err) {
      logger.error(`[registerRole:court] Court.create failed userId=${userId}: ${err.stack || err.message}`);
      throw new BizError(ErrorCode.PARAM_INVALID, `球场信息保存失败：${err.message || '数据库写入失败'}`);
    }
...
async function getUserProfile(req, res) {
  ...
  res.json(success({
    id: user.id, nickname: user.nickname, avatarUrl: user.avatarUrl, phone: user.phone, role: user.role, roles: user.roles || [], courtId: user.courtId,
    court: court && { id: court.id, name: court.name, address: court.address, district: court.district, type: court.type, surfaceType: court.surfaceType, surfaceTypes: court.surfaceTypes || [], status: court.status, openTime: court.openTime, closeTime: court.closeTime, openHours: court.openHours || null, createdAt: court.createdAt }
  }));
}
...
async function uploadAvatar(req, res) {
  ...
  await user.save();
  res.json(success({ id: user.id, avatarUrl }, '头像更新成功'));
}
